...
def process_layer_stream(config: ProcessConfig):
    ...
    num_workers = cpu_count() - 1  # Ein Core für das Hauptprogramm
    with Pool(num_workers) as pool:
        all_results = list(tqdm(
            pool.imap_unordered(process_single_file, [(f, config.rules) for f in valid_files]),
            total=len(valid_files),
            desc=f"  Parallel ({num_workers} Worker)",
            unit="tile"
        ))
    ...
    for rule in config.rules:
        ...
        for result in all_results:
            all_features.extend(result[rule.name])
